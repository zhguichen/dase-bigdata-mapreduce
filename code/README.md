...
| 节点角色 | 节点数量 | CPU    | 内存 | 存储 | 网络带宽 |
| -------- | -------- | ------ | ---- | ---- | -------- |
| Master   | 1        | 4 vCPU | 8 GB | SSD  | 1 Gbps   |
| Slave    | 3        | 4 vCPU | 8 GB | SSD  | 1 Gbps   |
...
![](https://cdn.nlark.com/yuque/0/2025/png/40512603/1764336828224-07c83d25-5907-47e1-9bd8-a3f1fc61635f.png)

+ 展示内容：在不同 `slowstart` 值下，倾斜（skewed）与均匀（uniform）两类数据的平均总耗时对比。
+ 主要结论：
  - 倾斜数据的总耗时曲线在较小 `slowstart` 区间更低，在晚启动时耗时较长。
  - 倾斜数据在“适度早启动”（通常 0.20 左右）处于较优；均匀数据的最优点更偏中位（通常 0.30–0.50）。
+ 原因与解释：
  - 关闭 Combiner 后，高频热键不再在 Map 端被聚合，Shuffle 与 Reduce 的真实负载更凸显。早一些启动 Reduce 能与 Map/Shuffle 更好重叠，降低等待；但过早或过晚都会让整体重叠度下降。
  - 均匀数据没有热点聚合收益，Shuffle 数据更平均且更大，总耗时曲线相对更高。

3. **Reduce 时间标准差**

![](https://cdn.nlark.com/yuque/0/2025/png/40512603/1764336850027-b12c2119-93af-4dc9-865a-acb51c090508.png)

+ 展示内容：不同 `slowstart` 值下，两类数据的 Reduce 完成时间标准差（越大表示任务间差异越明显）。
+ 主要结论：
  - 倾斜数据的标准差整体高于有 Combiner 场景，且在小 `slowstart` 区间保持偏高，表明长尾更显著。
  - 均匀数据在无 Combiner 条件下也会随 `slowstart` 变化出现标准差升高，但幅度与峰值通常低于倾斜数据的“真实倾斜”。
+ 原因与解释：
  - 倾斜数据的热键集中导致某个 Reduce 分区收到更多相同键，单任务的 Shuffle/聚合负载偏重，形成时间分布拉大。
  - `slowstart` 过早时，部分 Reduce 较长时间等待后续 Map 输出，早启动的“等待差异”也会增加标准差（伪倾斜成分）。

4. **长尾任务分析**

![](https://cdn.nlark.com/yuque/0/2025/png/40512603/1764336960594-f971e3bd-147b-4a05-9c04-aa53dbf965c7.png)

+ 展示内容：
  - (a) 长尾延迟：`Max - Avg`（最慢任务相对平均值的延迟）。
  - (b) 长尾比率：`Max / Min`（最大/最小耗时比）。
+ 主要结论：
  - 倾斜数据的长尾延迟与比率在无 Combiner 条件下显著提升（真实倾斜更明显），尤其在 `slowstart` 较小时更突出。
  - 均匀数据在早启动下也会出现一定的比率升高，但主要来自启动时机差异造成的等待并非负载不均（伪倾斜）。
+ 原因与解释：
  - 真实倾斜：热键集中→单个 Reduce 分区的 Shuffle/合并/计算更重→最慢任务拖尾增加。
  - 伪倾斜：早启动导致不同 Reduce 的“就绪时间”差异，形成完成时间的结构性分散。

5. **早启动有效性**

![](https://cdn.nlark.com/yuque/0/2025/png/40512603/1764337228747-fe4138f7-1734-4d41-8b8e-0eb5fabe19fe.png)

+ 展示内容：对比 `slowstart=0.05`（早启动）与 `slowstart=1.00`（晚启动）的总耗时差与节省比例。
+ 主要结论：
  - 在无 Combiner 条件下，早启动相对晚启动可显著降低总耗时；倾斜数据的受益更明显。
  - 但早启动并不能消除长尾（真实倾斜），只能部分缩短整体时间。
+ 原因与解释：
  - 早启动提升 Map/Shuffle/Reduce 的并行重叠度，减少空等时间；真实倾斜仍由键分布与分区策略主导，需要在数据与分区上治理。

6. **总结**

+ Combiner是应对数据倾斜的首选利器：当启用Combiner时，倾斜数据因Map端的局部聚合效应，Shuffle数据量大幅减少（降至均匀数据的40-50%），反而比均匀数据执行更快。这掩盖了数据倾斜的负面效应。
+ 无Combiner时，数据倾斜的真实影响暴露无遗：关闭Combiner后，倾斜数据表现出典型的“长尾”问题：总耗时增加、Reduce任务完成时间的标准差显著增大、长尾延迟和长尾比率飙升。
+ `slowstart` 参数是优化资源调度的有效工具：通过调整Reduce任务的启动时机，可以优化作业流程。实验表明，适度的“早启动”能有效提升整体性能，但无法根除由数据分布不均引起的“真实倾斜”。
